(function installV2Bridge() {
  'use strict';
  const marker=Symbol.for('openviking.v2.capture.installed');
  ...
  function emit(value) {
    ...
    if (++notified>12) {
      if(notified===13)globalThis.postMessage({type:'OV_V2_CAPTURE_V1',conversationId:value.conversationId,error:'capture_limit'},'https://chatgpt.com');
      return;
    }
    globalThis.postMessage({type:'OV_V2_CAPTURE_V1',...value},'https://chatgpt.com');
  }
  ...
  globalThis.fetch=function (...args) {
    ...
    if (info) promise.then(response=>{void observe(response,info).catch(()=>{});},()=>{}).catch(()=>{});
    return promise;
  };
})();
